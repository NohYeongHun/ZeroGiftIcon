...
import static com.zerogift.global.error.code.JwtErrorCode.EXPIRED_JWT;
import static com.zerogift.global.error.code.JwtErrorCode.INVALID_JWT_SIGNATURE;
...
import com.zerogift.support.auth.token.JwtInfo;
...
public class TokenService {
    ...
    private Claims parseToken(String token,byte[] encodedKey) {
        ...
        try {
            ...
            throw new JwtInvalidException(INVALID_JWT_SIGNATURE, e);
        } catch (MalformedJwtException e) {
            log.warn("Invalid JWT token: {}", e.getMessage());
            throw new JwtInvalidException(INVALID_JWT_SIGNATURE, e);
        } catch (ExpiredJwtException e) {
            log.warn("JWT token is expired: {}", e.getMessage());
            throw new JwtInvalidException(EXPIRED_JWT, e);
    ...
    public TokenDto refresh(String refreshToken) {
        ...
        if (!refreshTokenRepository.existsByUsername(email)) {
            throw new JwtInvalidException(EXPIRED_JWT);
        }
